...
import { allPages, allDocs } from '../../content';
...
let _allDocs = Object.fromEntries(allDocs.map(({ id, slug, data }) => [id, { data, slug }]));
let _allPages = Object.fromEntries(allPages.map(({ id, slug, data }) => [id, { data, slug }]));

const pages = { ..._allDocs, ..._allPages };

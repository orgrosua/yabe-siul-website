...
During development, Yabe Siul uses the [Play CDN](https://tailwindcss.com/docs/installation/play-cdn) to dynamically create the required page styles. This approach is fast and straightforward. However, it is not suitable for use in production environments.

For the most performance in the production environment, Yabe Siul will use engine running right on your browser to generate a tiny single cached CSS file. We use [Lightning CSS](https://lightningcss.dev/) to minify CSS for the smallest possible build. None of your data is transferred over the network. Everything is done on your browser.
